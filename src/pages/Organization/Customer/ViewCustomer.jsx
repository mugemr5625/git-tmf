...
const ViewCustomer = () => {
  ...
  const fetchAreaData = async () => {
    try {
      ...
      if (response?.status === 200) {
        ...
        // Extract unique branches and lines
  ...
  const handleApplyFilter = () => {
    ...
    // Filter customers based on branch and line through areas
    const filteredAreas = areas.filter(
      (area) => area.branch_name === tempBranch && area.line_name === tempLine
    );
    const areaIds = filteredAreas.map((area) => area.id);

    // const filtered = originalCustomers.filter((customer) =>
    //   areaIds.includes(customer.area)
    // );
    const filtered = originalCustomers;
  ...
  const CustomerCollapseContent = ({ customer }) => {
    const areaName = areaIdToNameMap[customer.area] || `Area ${customer.area}`;
    return (
